...
async def predict(body: PredictionRequest):
    ...
    try:
        ...
        # В секции формирования контекста
        context = "\n".join(
            [f"Источник {i+1}: {res['content'][:500]}"  # Ограничение длины контента
        ...
        if settings.MISTRAL_API_KEY:
            ...
            response_data["answer"] = int(llm_response.get(
                "answer")) if llm_response.get("answer") else None
        ...
        response_data["sources"] = [res["url"]
                                    for res in search_results[:3] if isinstance(res.get("url"), str)]
    ...
    return Response(
        # Отключаем ASCII-кодирование
        content=json.dumps(response_data, ensure_ascii=False),
        media_type="application/json; charset=utf-8",  # Явно указываем кодировку
        status_code=200
    )
